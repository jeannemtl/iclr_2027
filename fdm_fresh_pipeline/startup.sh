#!/bin/bash
# ============================================================
# FDM project pod startup script
#
# Covers everything done manually in this session:
#   1. SSH rehydration (if rehydrate.sh exists on this volume)
#   2. Python deps (huggingface_hub, transformers, torch)
#   3. HF auth (interactive — you'll be prompted for a token)
#   4. Download of the four confirmed model checkpoints:
#        GPT-2, Qwen3, LFM2.5, Hermes3
#   5. Download of the matching four test-data dataset repos
#   6. Writes the corrected fdm_common.py (Meta-scoring bugfix)
#        to /workspace/fdm_common.py
#
# NOTE: on this session's fresh volume, /workspace/rehydrate.sh
# did not exist (empty volume, not the persistent one with prior
# data). Step 1 below checks for it and skips gracefully if
# missing rather than failing the whole script — but if your real
# persistent volume is supposed to be mounted here, confirm that
# BEFORE running this, since a missing rehydrate.sh may mean the
# wrong volume is attached rather than that rehydration is simply
# unnecessary.
#
# Usage:
#   chmod +x startup.sh
...
set -e  # exit on first error (comment out if you'd rather continue past failures)

echo "============================================================"
echo "STEP 1: SSH rehydration"
echo "============================================================"
cd /workspace
if [ -f /workspace/rehydrate.sh ]; then
    bash rehydrate.sh
    ls -la ~/.ssh/
    cat ~/.gitconfig 2>/dev/null | grep sshCommand || echo "  (no sshCommand set in .gitconfig)"
else
    echo "  /workspace/rehydrate.sh not found on this volume."
    echo "  Skipping SSH rehydration. If you expected your persistent"
    echo "  volume (with prior checkpoints, keys, etc.) to be attached"
    echo "  here, STOP and check the RunPod volume selection — this"
    echo "  looks like a fresh/empty volume, not your existing one."
fi
...
echo "============================================================"
pip install -U huggingface_hub transformers torch --break-system-packages 2>/dev/null || \
pip install -U huggingface_hub transformers torch

echo ""
echo "============================================================"
echo "STEP 3: Hugging Face auth"
echo "============================================================"
echo "  You will be prompted to paste your HF token."
hf auth login

echo ""
echo "============================================================"
echo "STEP 4: Download model checkpoints (4 architectures)"
echo "============================================================"
echo "  GPT-2 ..."
hf download prompterminal/fdm_40ch_turbo_v3_model_final --local-dir /workspace/models/gpt2

echo "  Qwen3 (stages repo -- using final/) ..."
hf download prompterminal/fdm-40ch-turbo-v3-qwen3-stages --local-dir /workspace/models/qwen3_stages

echo "  LFM2.5 ..."
hf download prompterminal/fdm-40ch-turbo-v3-lfm2 --local-dir /workspace/models/lfm2

echo "  Hermes3 ..."
hf download prompterminal/fdm-40ch-turbo-v3-hermes3-1024tok --local-dir /workspace/models/hermes3

echo ""
echo "============================================================"
echo "STEP 5: Download matching test datasets (4 architectures)"
echo "============================================================"
echo "  GPT-2 data ..."
hf download --repo-type dataset prompterminal/fdm-40ch-turbo-v3-gpt2 --local-dir /workspace/data/gpt2

echo "  Qwen3 data ..."
hf download --repo-type dataset prompterminal/fdm-40ch-turbo-v3-qwen3 --local-dir /workspace/data/qwen3

echo "  LFM2.5 data ..."
hf download --repo-type dataset prompterminal/fdm-40ch-turbo-v3-lfm2 --local-dir /workspace/data/lfm2

echo "  Hermes3 data ..."
hf download --repo-type dataset prompterminal/fdm-40ch-turbo-v3-hermes3-1024tok --local-dir /workspace/data/hermes3

echo ""
echo "============================================================"
echo "STEP 6: Verify checkpoint architectures (sanity check)"
echo "============================================================"
python3 << 'PYEOF'
import json, os

checks = {
    "gpt2":   ("/workspace/models/gpt2/config.json", "gpt2"),
    "qwen3":  ("/workspace/models/qwen3_stages/final/config.json", "qwen3"),
    "lfm2":   ("/workspace/models/lfm2/config.json", "lfm2"),
    "hermes3": ("/workspace/models/hermes3/config.json", "llama"),
}

for name, (path, expected_type) in checks.items():
    if not os.path.exists(path):
        print(f"  [{name}] MISSING: {path}")
        continue
    c = json.load(open(path))
    actual_type = c.get("model_type")
    vocab = c.get("vocab_size")
    status = "OK" if actual_type == expected_type else "MISMATCH"
    print(f"  [{name}] model_type={actual_type} (expected {expected_type}) vocab_size={vocab}  -> {status}")
PYEOF

echo ""
echo "============================================================"
echo "STEP 7: Write corrected fdm_common.py"
echo "============================================================"
cat > /workspace/fdm_common.py << 'PYEOF'
"""
...
echo "  Written: /workspace/fdm_common.py"
...
echo ""
echo "Checkpoints:"
echo "  GPT-2:    /workspace/models/gpt2"
echo "  Qwen3:    /workspace/models/qwen3_stages/final"
echo "  LFM2.5:   /workspace/models/lfm2"
echo "  Hermes3:  /workspace/models/hermes3"
echo ""
echo "Test data (stage4_test.jsonl is what Table 3 used):"
echo "  GPT-2:    /workspace/data/gpt2/fdm_40ch_turbo_v3_gpt2_stage4_test.jsonl"
echo "  Qwen3:    /workspace/data/qwen3/fdm_40ch_turbo_v3_qwen3_stage4_test.jsonl"
echo "  LFM2.5:   /workspace/data/lfm2/fdm_40ch_turbo_v3_lfm2_stage4_test.jsonl"
echo "  Hermes3:  check exact filename in /workspace/data/hermes3/ (1024tok variant)"
echo ""
echo "Run corrected eval, e.g. for Qwen3:"
echo ""
echo "  cd /workspace && python3 -c \""
echo "  import torch"
echo "  from transformers import AutoModelForCausalLM, AutoTokenizer"
echo "  from fdm_common import evaluate_model"
echo "  tok = AutoTokenizer.from_pretrained('/workspace/models/qwen3_stages/final', trust_remote_code=True)"
echo "  model = AutoModelForCausalLM.from_pretrained('/workspace/models/qwen3_stages/final', torch_dtype=torch.bfloat16, trust_remote_code=True).cuda().eval()"
echo "  evaluate_model(model, tok, '/workspace/data/qwen3/fdm_40ch_turbo_v3_qwen3_stage4_test.jsonl', 'cuda', 'qwen3')"
echo "  \""
echo ""
echo "Repeat for gpt2 (torch.float32, GPT2LMHeadModel/GPT2Tokenizer),"
echo "lfm2, and hermes3 -- swap the paths and model_short label."
echo ""
echo "REMINDER: fdm_40ch_turbo_v3_model_final is GPT-2, NOT Qwen3,"
echo "despite the generic-sounding name (confirmed: model_type=gpt2)."
echo "fdm-40ch-turbo-v3-qwen3-1024tok is EMPTY (.gitattributes only) --"
echo "do not use it; the qwen3-stages/final/ checkpoint above is the"
echo "verified real Qwen3 checkpoint."

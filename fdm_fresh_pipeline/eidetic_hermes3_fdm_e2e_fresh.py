"""
Eidetic Memory FDM — Hermes3-3B, FRESH matched pipeline
...
MODEL_ID/VOCAB_SIZE confirmed from nhop_training_multi.py's
MODEL_CONFIGS in this session:
  model_id: "NousResearch/Hermes-3-Llama-3.2-3B"
  vocab_size: 128256

Usage:
  python eidetic_hermes3_fdm_e2e_fresh.py generate
  python eidetic_hermes3_fdm_e2e_fresh.py train
  python eidetic_hermes3_fdm_e2e_fresh.py eval [stage]
"""
...
GRAD_ACCUM_STEPS = 2
MAX_LENGTH = 768
OUTPUT_PREFIX = "fdm_40ch_fresh_hermes3"
CHECKPOINT_DIR = "checkpoints_fdm_fresh_hermes3"
FINAL_MODEL_DIR = "fdm_40ch_fresh_hermes3_model_final"

# Project notes (two-stream curriculum) state Hermes3 requires
# per-stage learning rates scaled by 1/3 for stable training at 3B
# scale. Applied here for the single-block curriculum too, since the
# same base LRs (5e-5 down to 1e-5) are used and 3B-scale instability
# is not stage-specific. If single-block training diverges without
# this scaling in practice, that would itself be worth noting; if it
# trains fine unscaled, this can be set back to 1.0.
LR_SCALE = 1.0 / 3.0
...
def train():
    ...
    print(f"LR scale factor: {LR_SCALE}")
